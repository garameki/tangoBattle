...
Canv.prototype.puwa = function() {
	this.ctx.fillStyle = 'magenta';
	let radius = 0;
	const hogePuwa = setInterval(()=>{
		radius++;
		this.ctx.beginPath();
		this.ctx.arc(5,5,radius,0,2*Math.PI);
		this.ctx.fill();
		if(radius>10) clearInterval(hogePuwa);
	},500);
};
...
Canv.MIDDLE = 1;
Canv.BOTTOM = 2;
Canv.prototype.appearElement = function(sFilename,time,posTB,posLR) {	
	let nn,sTop,sLeft;
 ...
 ** 0.1秒間隔で時間を指定できます
 ***********************************************************/
Canv.prototype.disappear = function(time) {
...
**************************************************/
Canv.prototype.move = function(xx,yy,time) {
	let sLeft='',sTop='';
	const div = 10;/*コマの数*/
	const beforeTop = this.style.top;
	const top = Number(beforeTop.match(/\-*\d+(?:\.\d+)*/)[0]);
